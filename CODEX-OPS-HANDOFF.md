...
`codex-ruoyu` 位于 `/home/allen/.local/bin/codex-ruoyu`，权限应为 `0700`。它只把干净的基础环境交给 Codex，并设置 `CODEX_HOME=/home/allen/.codex`；模型、provider 与 base URL 只由 `/home/allen/.codex/config.toml` 管理，Codex CLI 自己读取原生登录态 `/home/allen/.codex/auth.json`。不再读取额外的 provider 环境文件。
