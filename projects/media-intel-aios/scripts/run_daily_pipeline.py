...
try:
    import article_production_live_run as article_live_run  # type: ignore[import-not-found]  # noqa: E402
except ModuleNotFoundError:
    article_live_run = None  # type: ignore[assignment]
...
def article_backfill_bundle_key(row: dict[str, Any]) -> str:
    text_candidates = [
        row.get("work_title"),
        row.get("film_title"),
        row.get("drama_title"),
        row.get("related_title"),
        row.get("title"),
    ]
    text = merged_text(*text_candidates)
    match = re.search(r"《([^》]{2,32})》", text)
    if match:
        return match.group(1)
    normalized = re.sub(r"[^\u4e00-\u9fffA-Za-z0-9]+", "", compact_text(text, limit=40))
    return normalized[:24]


def article_backfill_signal_role(row: dict[str, Any]) -> str:
    source = str(row.get("source") or row.get("source_name") or "").lower()
    text = article_text(row)
    if row.get("source_type") == "article_body" or "toutiao" in source or "头条" in source or "正文" in source:
        return "article_body_signal"
    if row.get("source_type") == "commentary" or "douban" in source or "豆瓣" in source or "小红书" in source:
        return "audience_reaction_signal"
    if "weibo" in source or "微博" in source or "bilibili" in source or "b站" in source or "douyin" in source or "抖音" in source or "zhihu" in source or "知乎" in source:
        return "social_discussion_signal"
    if any(term in text for term in ["短评", "评论", "观众", "争议", "吵"]):
        return "audience_reaction_signal"
    return "unknown_signal"


def build_article_evidence_bundles(article_rows: list[dict[str, Any]], date: str) -> list[dict[str, Any]]:
    grouped: dict[str, list[dict[str, Any]]] = {}
    for row in article_rows:
        if row.get("evidence_bundle_generated"):
            continue
        if row.get("body_fetch_status") != "live_signal_backfill":
            continue
        if article_is_discovery_source(row):
            continue
        key = article_backfill_bundle_key(row)
        if len(key) < 2:
            continue
    ...
    for key, rows in grouped.items():
        source_names = list(dict.fromkeys(str(row.get("source_name") or row.get("source") or "未知来源") for row in rows))
        signal_roles = list(dict.fromkeys(article_backfill_signal_role(row) for row in rows if article_backfill_signal_role(row) != "unknown_signal"))
        if len(source_names) < 2 or len(signal_roles) < 2:
            continue
        selected = sort_article_rows([enrich_article_row_for_scoring(dict(row), date) for row in rows])[:5]
        titles = [compact_text(row.get("title") or "无标题", limit=80) for row in selected]
        evidence_lines = [f"{source_names[idx] if idx < len(source_names) else row.get('source_name')}: {compact_text(row.get('summary') or row.get('content') or row.get('title') or '', limit=120)}" for idx, row in enumerate(selected)]
        primary = selected[0]
        content = "\n".join(filter(None, [
            f"《{key}》多源证据包。",
            "今日钩子：live backfill 聚合到至少两个叙事信号源，包含观众反应、社交讨论或正文结构信号。",
            "争议/讨论：" + "；".join(titles[:3]),
            "证据摘要：" + "；".join(evidence_lines[:5]),
            compact_text(primary.get("content") or primary.get("summary") or "", limit=500),
        ]))
        bundles.append(enrich_article_row_for_scoring({
            "source": "live_backfill_bundle",
            "source_name": "live backfill 多源证据包",
            "source_type": "evidence_bundle",
            "content_type": "article_body",
            "title": f"《{key}》的观众争议和今日讨论信号",
            "summary": f"聚合 {len(source_names)} 个来源、{len(signal_roles)} 类叙事信号：{', '.join(signal_roles)}。",
            "content": content,
            "rank_date": date,
            "publish_time": date,
            "sample_evidence": "multi_source_live_backfill_bundle",
            "article_route_reason": "live backfill produced a multi-source evidence bundle; eligible for writer gate only after normal admission checks",
            "body_fetch_status": "multi_source_evidence_bundle",
            "evidence_bundle_generated": True,
            "evidence_bundle_key": key,
            "evidence_bundle_source_count": len(source_names),
            "evidence_bundle_signal_roles": signal_roles,
            "evidence_bundle_titles": titles,
            "evidence_bundle_sources": source_names,
        }, date))
    return bundles
...
def run_article_retry_loop(
    ...
    refetched_rows = list(refetcher(initial_retry_request) or [])
    refetched_scoring_rows = [enrich_article_row_for_scoring(dict(row), date) for row in refetched_rows]
    evidence_bundle_rows = build_article_evidence_bundles(refetched_scoring_rows, date)
    combined_rows = article_rows + refetched_scoring_rows + evidence_bundle_rows
    ...
    return {
        ...
        "evidence_bundle_count": len(evidence_bundle_rows),
...
def candidate_profile(row: dict[str, Any], kind: str) -> dict[str, Any]:
    if kind == "article":
        ...
        if row.get("body_fetch_status") == "multi_source_evidence_bundle":
            source_count = int(row.get("evidence_bundle_source_count") or 0)
            role_count = len(row.get("evidence_bundle_signal_roles") or [])
            if source_count >= 2 and role_count >= 2 and compact_text(row.get("content") or ""):
                evidence_score = max(evidence_score, 82)
                topic_score = max(topic_score, 72)
...
def build_article_approved(date: str, article_rows: list[dict[str, Any]]) -> str:
    ...
    for row in ranked_rows:
        ...
        profile = candidate_profile(row, "article")
        readiness = profile.get("write_readiness")
        gate = profile.get("gate")
        readiness_block = ""
        if gate != "pass":
            readiness_block = f"profile_gate={gate}; {profile.get('reason') or 'not eligible for approved article output'}"
        elif readiness != "write_ready_candidate":
            readiness_block = f"write_readiness={readiness}; keep in evidence backfill/incubation, not approved output"
        if hard_reason or readiness_block:
            reason = "; ".join(part for part in [hard_reason, readiness_block] if part)
            if admission["production_score"] >= int(POOL_THRESHOLDS.get("B", 55)) or readiness in {"needs_evidence_backfill", "topic_hot_candidate"}:
                blocked_rows.append((row, reason))
            continue
...
def main() -> int:
    ...
    def refetch_article_rows(retry_request: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        evidence_tasks = retry_request.get("evidence_backfill_tasks") or []
        live_queries = [str(task.get("query") or "").strip() for task in evidence_tasks if str(task.get("query") or "").strip()]
        live_queries.extend(str(query or "").strip() for query in (retry_request.get("queries") or []) if str(query or "").strip())
        live_queries = list(dict.fromkeys(query for query in live_queries if query))
        if article_live_run is not None and live_queries:
            live_rows: list[dict[str, Any]] = []
            for query in live_queries[:3]:
                try:
                    live_rows.extend(article_live_run.collect_live_backfill_rows(query=query))
                except Exception:
                    continue
            if live_rows:
                refetched_article_rows.extend(live_rows)
