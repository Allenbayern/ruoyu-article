...
CONSTRAINTS: The batch must declare `run_profile` and `run_profile_contract_version: run-profile-v1`; `two_article_daily` has A/B, `three_slot_controlled` has A/B/C. Each Markdown draft uses the style-gate CJK count: 1500-2200 Chinese characters is the usual target, with a 1000-2700 flexible acceptance band when the article is complete and readable; no social/discovery signal is the sole support for a material claim; every article remains `not_authorized` for publication.
ACCEPTANCE:
...
- Each declared Markdown, evidence pack, and writing brief is non-empty and inside the run root; Markdown is actually read and contains 1000–2700 Chinese characters, with 1500–2200 as the usual target.
