...
def main():
 ...
 for name in ("init","record","verify"):
  ...
  if name == "verify":
   ...
   s.add_argument("--v4-source-dir",type=Path); s.add_argument("--v5-source-dir",type=Path)
 ...
 if a.cmd=="verify":
  out["context"]=verify_context(v4_run_dir=a.v4_run_dir,v5_run_dir=a.v5_run_dir,v4_source_dir=a.v4_source_dir,v5_source_dir=a.v5_source_dir,expected_contexts=manifest.get("verification_contexts"))
  out["status"]="BLOCKED" if out["context"]["status"]!="PASS" else out["status"]
