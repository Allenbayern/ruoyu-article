...
Codex 使用自己的登录态和配置入口，不复制 Hermes 的认证文件。已批准的本机桥接是 `/home/allen/.local/bin/codex-ruoyu`；默认模式由 Codex CLI 自己读取 `/home/allen/.codex/auth.json`，provider 模式只在子进程环境中允许明确的 `OPENAI_BASE_URL`、`OPENAI_API_KEY`、`OPENAI_MODEL` 名称。实际值始终视为 `[REDACTED]`，不得进入 prompt、argv、项目文件、manifest、日志或聊天。
