...
> **层状态（2026-09-15 裁定）**：本 1.0 总控契约（`article_task_v1` 的
> article-task-v1 / crawl-task-parent-v1 / article-group-v1）是 task-hierarchy
> 门禁的现行基准。日更生成器必须产出本契约形状，且每个 run 必须通过
> `validate_task_hierarchy_run`（见 `task-hierarchy-validation-report.json`）。
> 层分工见 `article_group/__init__.py` 顶部：v2_contract 已冻结；v4/v5 为旁路
> 工具层；`schemas/editorial-pipeline-v3` 是与本契约并存的 pipeline schema 族。
...
新建的 article-first run 必须在 run 根（`batch.json`）声明唯一契约：
...
契约字段缺失、取值不一致，或把旧 `Title`/`title_promise` 等字段带入前正文阶段，
直接返回 `contract_mismatch`。历史兼容也必须在 run 根明确写
`legacy_compatibility: true`；只要出现 article-first 产物而两种 lane 都没有声明，
同样返回 `contract_mismatch`，不得靠字段形状猜测它是历史记录。
...
严格 run 还必须提交 `core_judgment`、至少两条 `judgment_basis`、
`judgment_strength`、`reader_can_repeat` 和 `unsupported_scenario_boundary`。两条依据
必须分别定位到事实或场面并解释它们如何支持同一个判断；`too_strong`、`too_weak` 或
不可转述的判断不能通过。
...
严格材料验收同时输出 `material_ready_for_draft` 与 `editorial_value_ready`；两者都为
真才可进入写作。来源声明最大叙事能力，claim 声明 `claim_level`，并逐条检查
`claim_level <= source_capability`。简介最多支撑事件存在或人物设定，不能自动扩写为
具体动作、对白、观众反应、解释机制或结果。
...
严格独立复核还要绑定 `artifact_path/artifact_sha256`、`body_path/body_sha256`、
`title_pack_path/title_pack_sha256` 和 `created_from_run`。提交时重新计算，任一不一致
均为 `stale_review`，不能用人工备注覆盖。总复核报告固定分开输出
`content_result`、`evidence_result`、`governance_result` 和
`publication_authorization`；最后一项仍为 `not_authorized`。

终审必须落盘为 `review/final-review.json`，并绑定当前 `batch.json` 的 SHA-256、完整
`run_id` 和当前审阅面文件的 SHA-256。交付入口会重新执行终审并比较这些绑定，不能用
调用方传入的 `PUBLISHABLE` 报告替代终审。
