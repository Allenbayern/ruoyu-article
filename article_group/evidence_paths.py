...
- 自动识别 run 根时额外要求它是规范形状（``…/runs/<date>/<id>``，即父目录名为 ``runs``）。
  ``run_seal.is_run_root`` 只看路径里**第一个** ``runs`` 组件，遇到"runs 之前还有 runs"
  的布局会把外层目录误判成 run 根——实测那样写出来的相对路径原地就 BLOCKED（回归）；
- 文件正好落在 ``runs/<X>/<文件>`` 时，``is_run_root`` 会把文件本身当成 run 根
  （仓库里有 58 个这种形状的文件）→ 由"根必须是产物的真祖先"挡掉。
...
def _canonical_run_root(path: Path) -> Path | None:
    ...
    为什么要这么严（2026-09-18 L2 复核实测）：``run_seal.is_run_root`` 只看路径里**第一个**
    ``runs`` 组件、且不校验日期与目录，于是

    - 文件正好落在 ``runs/<X>/<文件>`` 时它把**文件**当 run 根 → 记出 ``"."``；
    - 路径里"runs 之前还有 runs"时它把外层目录当 run 根 → 记出错误相对路径，
      同一个 run 原地 evaluate 就 BLOCKED（相对绝对路径是回归）。

    自动识别宁可落回绝对路径，也不能记一个错的相对路径；显式 ``run_root=`` 不受这条限制
    （调用方知道自己的布局，例如 daily_engine）。
    """
