...
_READING_REPORT_TONE: list[tuple[str, str, str]] = [
    ...
    # 2026-09-21 助手残留与客服式设问（反AI编辑层）
    (r"希望对你有帮助", "tone:助手残留", "AI助手残留：'希望对你有帮助'"),
    (r"如果需要我可以", "tone:助手残留", "AI助手残留：'如果需要我可以'"),
    (r"欢迎在评论区(留言|讨论|分享)", "tone:自媒体套路句", "自媒体套路句：'欢迎在评论区留言'"),
    (r"欢迎(在下方)?留言", "tone:自媒体套路句", "自媒体套路句：'欢迎留言'"),
    (r"你会如何看待", "tone:客服设问句", "客服设问句：'你会如何看待'属于机器设问残留"),
    (r"你怎么看待", "tone:客服设问句", "客服设问句：'你怎么看待'属于机器设问残留"),
]
...
def closing_interaction_check(paragraphs: list[str]) -> dict[str, Any]:
    ...
    return {
        "status": "info",
        "reason": ("末段以事实或行业判断定格（结论性收尾，合规）；"
                   "深度长文优先保持专业定力，避免为了互动添加套路化设问"),
        "last_40": last[-40:],
    }
